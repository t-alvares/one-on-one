...
import { ThoughtsIcon, ICON_SIZES } from '@/components/icons';
import {
  useLeaderThoughts,
  useDeleteLeaderThought,
  useUpdateLeaderThought,
  useCreateLeaderThought,
  type Thought,
} from '../../hooks/useLeaderThoughts';
import { useICContext } from '../../contexts/ICContext';
import { ThoughtCard } from '../thoughts/ThoughtCard';

interface LeaderThoughtPanelProps {
  /** Additional class names */
  className?: string;
}

/**
 * LeaderThoughtPanel Component
 * Displays Leader's thoughts ABOUT a specific IC
 * - Uses ICContext to get current IC's ID
 * - Creates thoughts with aboutIcId set to current IC
 */
export function LeaderThoughtPanel({ className = '' }: LeaderThoughtPanelProps) {
  ...
  const { data: thoughts = [], isLoading, error, refetch } = useLeaderThoughts(icId);
  const deleteThought = useDeleteLeaderThought(icId);
  const updateThought = useUpdateLeaderThought(icId);
  const createThought = useCreateLeaderThought();
  ...
  const handleTitleUpdate = useCallback(
    (thought: Thought, newTitle: string) => {
  ...
  const handleLabelUpdate = useCallback(
    (thought: Thought, labelId: string | null) => {
  ...
  const handleNavigate = useCallback(
    (thought: Thought) => {
      navigate(`/team/${icId}/thoughts/${thought.id}`);
    },
    [navigate, icId]
  );
  ...
  const handleAddThought = useCallback(async () => {
    ...
    try {
      await createThought.mutateAsync({ title: trimmed, aboutIcId: icId });
      setNewThoughtTitle('');
      setIsAddingThought(false);
    } catch (err) {
      console.error('Failed to create thought:', err);
    }
  }, [newThoughtTitle, createThought, icId]);
  ...
  const handleDragOver = useCallback(
    (_event: React.DragEvent, thought: Thought) => {
        ...
        <div className="relative flex-1 min-h-0 overflow-y-auto">
          ...
          {thoughts.length === 0 && !isAddingThought && (
            <p className="text-sm text-text-tertiary py-2">
              No thoughts yet. Capture quick notes about {ic?.name || 'this person'}.
            </p>
          )}
...
export type { LeaderThoughtPanelProps };
