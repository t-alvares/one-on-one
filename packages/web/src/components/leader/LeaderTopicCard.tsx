...
import { SquarePen, GripVertical, Tag, Calendar, Loader2, Users } from 'lucide-react';
import { format } from 'date-fns';
import type { Topic } from '../../hooks/useLeaderTopics';
...
interface LeaderTopicCardProps {
  /** The topic to display */
  topic: Topic;
  /** Delete handler (only for leader's own topics) */
  onDelete?: (topic: Topic) => void;
  /** Title update handler for inline editing (only for leader's own topics) */
  onTitleUpdate?: (topic: Topic, newTitle: string) => void;
  /** Label update handler (only for leader's own topics) */
...
/**
 * LeaderTopicCard Component
 * Extended TopicCard for Leader's IC workspace
 * - If isCounterparty: read-only, lighter background, shows "From [IC Name]"
 * - If own topic: full edit capabilities
 */
export function LeaderTopicCard({
  topic,
  onDelete,
  onTitleUpdate,
  onLabelUpdate,
  ...
  // Counterparty topics are read-only
  const isReadOnly = topic.isCounterparty;
  ...
  const handleDragStart = (e: React.DragEvent) => {
    if (isReadOnly) return; // Can't drag counterparty topics
  ...
  const handleDrop = (e: React.DragEvent) => {
    ...
    // For thought drops: don't preventDefault or stopPropagation - let it bubble to LeaderTopicPanel
  };
  ...
  const getStatusBadge = () => {
    switch (topic.status) {
      ...
      case 'DISCUSSED':
        return <span className="text-xs text-success">Discussed</span>;
      case 'ARCHIVED':
        return <span className="text-xs text-text-tertiary">Archived</span>;
      default:
  ...
  return (
    <div
      draggable={isDraggable && !isEditing && !isReadOnly}
      ...
      `}
    >
      {/* Circle marker on the line - different style for counterparty */}
      <div
        className={`
          relative z-10 mt-1.5 w-[11px] h-[11px] rounded-full border flex-shrink-0 transition-colors
          ${isReadOnly ? 'border-gray-300 bg-gray-100' : 'border-border bg-white group-hover:border-text-tertiary'}
        `}
      />

      {/* Content */}
      <div className="flex-1 min-w-0 flex items-start justify-between gap-2">
        {/* Title, label, and status */}
          ...
          {isEditing && !isReadOnly ? (
            ...
            <p
              ...
              className={`
                ...
                ${isReadOnly ? 'text-text-secondary' : 'text-text-primary'}
                ${onTitleUpdate && !isReadOnly ? 'cursor-text' : ''}
              `}
            >
              {/* Counterparty indicator */}
              {isReadOnly && (
                <span className="inline-flex items-center gap-1 text-xs text-text-tertiary mr-2">
              ...
              {topic.status !== 'BACKLOG' && (
                <span className="ml-2 align-baseline">{getStatusBadge()}</span>
              )}
            </p>
          )}
        </div>

        {/* Actions: only show for own topics (not counterparty) */}
        {!isReadOnly && (
          <div className="flex items-center gap-1 flex-shrink-0">
              ...
              <div className="relative" ref={labelPickerRef}>
                ...
                {showLabelPicker && (
                  <div className="absolute right-0 top-full mt-1 z-50 bg-white border border-border/50 rounded shadow-sm py-1 min-w-[100px]">
                    ...
                    {labels.length === 0 && (
                      <div className="px-2.5 py-1 text-xs text-text-tertiary">No labels</div>
                    )}
                  </div>
                )}
              </div>
            ...
            {/* Edit - navigate to topic detail page */}
            <button
              type="button"
              onClick={() => navigate(`/team/${icId}/topics/${topic.id}`)}
              className="
                p-1 rounded
                text-text-tertiary
      ...
      <Modal isOpen={showDeleteModal} onClose={() => setShowDeleteModal(false)} title="Delete Topic" size="sm">
        <p className="text-sm text-text-secondary mb-2 -mt-2">Are you sure you want to delete "{topic.title}"?</p>
...
export type { LeaderTopicCardProps };
